...
//ovaj broj jer je deljiv sa 1024 a prvi je koji ide posle 101*640*3*2
#define MAX_PKT_LEN			        389120 
...
i	
	...
	if(strcmp(comp, "dma_ip") == 0) device_fsm = 0;
	if(strcmp(comp, "title_ip") == 0) device_fsm = 1;
	...
	case 0:
        ...
        if(!dma_p->irq_num0 || !dma_p->irq_num1)
		{
		...
		if (request_irq(dma_p->irq_num0, dma_MM2S_isr, 0, DEVICE_NAME, dma_p)) {
		...
		if (request_irq(dma_p->irq_num1, dma_S2MM_isr, 0, DEVICE_NAME, dma_p)) {
			printk(KERN_ERR "[title_probe] Could not register IRQ %d\n", dma_p->irq_num1);
			return -EIO;
			goto error7;
		}
...
{   
	...
	case 0:
        if(endRead)
        {
            printk(KERN_WARNING "[title_read] Succesfully read from TITLE\n");
            endRead = 0;
            return 0;
        }
	    len = scnprintf(buff, BUFF_SIZE, "%d %d", ip_frame_over, ip_command_over);
...
ssize_t title_write(struct file *pfile, const char __user *buf, size_t length, loff_t *offset)
{
	...
	switch(minor)
	{
		...
		case 0:
            ...
            sscanf(buff, "%d,%d,%d", &input_command, &dimension, &offset_com_pos);  
		    printk(KERN_INFO "comm:%x, dim:%d, off:%d\n" , input_command, dimension, offset_com_pos);
            ...
            else if(offset_com_pos == 1)
            {   
                printk(KERN_INFO "[title_write] OFFSET = 1\n");
...
static int title_mmap(struct file *f, struct vm_area_struct *vma_s)
{
	...
	//printk(KERN_INFO "[title_dma_mmap] Length:%ld\n", length);
